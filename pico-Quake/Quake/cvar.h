...
//To create the forwardly linked list
typedef struct cvar_node_s{
	cvar_t cvar;
	struct cvar_node_s* next;
} cvar_node_t;

//The core set of Cvars are no longer a forwardly linked list, but instead are an array of fixed length
//Additional Cvars may be added in a forwardly linked list.
#define CORE_CVAR_LENGTH 25
struct {
